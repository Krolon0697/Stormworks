...
---@section KL_Harmonica 1 KL_HARMONICACLASS
KineticaL.KL_Harmonica={
	---@section KL_convolutions 1 KL_CONVOLUTIONCLASS
	KL_convolutions={
		---@section KL_fftConvolve
		...
		KL_fftConvolve=function(signalIn,kernelIn,fftModule)
			if UniqueNameFFTConvolveMemoryKrolon==nil then UniqueNameFFTConvolveMemoryKrolon={signal={},kernel={}} end
			local typeS,typeK,fallback,mem,sR,sIm,kR,kIm=math.type(signalIn[1]),math.type(kernelIn[1]),{real=0},UniqueNameFFTConvolveMemoryKrolon
			local signal,kernel,N=mem.signal,mem.kernel,#signalIn+(typeS and 0 or 1)+#kernelIn+(typeK and 0 or 1)-1
		...
		KL_createWavelet=function(f,N,width)
		...
		---@section KL_genWaveletKernelSize
		...
		KL_pruneEnds=function(t,startAt,endAt)
	...
	---@endsection KL_CONVOLUTIONCLASS

	---@section KL_getAmplitudes
    ...
    KL_getAmplitudes=function(fhat)
	...
	KL_fftUniRadix=function(input,inverse,disableAutoNormalization,customRadices)

		--localize shit for user safety
		if UniqueNameFFTCRMemoryKrolon==nil then UniqueNameFFTCRMemoryKrolon={space={{},{}},orders={},[true]={},[false]={},possible_factors={2,3,5,7}} end
		local factors,N,inverse,mem,factor,twiddles,helper1,helper2,oldLayer,newLayer,V,A,B,Ar,Aim,Br,Bim,r,im,permutation,size,fftSize,create_size_cache={},#input+(input[0] and 1 or 0),inverse or false,UniqueNameFFTCRMemoryKrolon

		fftSize=function()
			--imperfect but compact-ish and very fast way of finding (well, not so) closest next possible N size
			local possiblefactors,t_insert,ipairs,check=customRadices or mem.possible_factors,table.insert,ipairs
			while N>1 do
				check=true
				for i,factor in ipairs(possiblefactors) do
					if N%factor==0 then
						check=false
						N=N/factor
			...
			for k=#factors,2,-1 do
				...
				for j=0,N/V-1 do
					for i=0,V-1 do
						...
						helper1[i+j*V+1]=permutation[A] or A --or A avoid having to initialize the table. If there's no table - use the index as value
					end
				end
		...
		--N,factors=fftSize(N)
		fftSize()
		permutation=mem.orders[N] or create_size_cache()
		...
		B=(inverse or disableAutoNormalization) and 1 or 1/N --perform a normalization already on inputs when doing forward transform
		factor={}
		for i=1,N do
	...
	---@param disableAutoNormalization boolean default false, flag to disable normalization (which happens only on forward transform)
	---@return table complex complex table, FFT output
	KL_fftRadix2=function(input,inverse,disableAutoNormalization)

		--localize shit for user safety
		if UniqueNameFFTMemoryKrolon==nil then UniqueNameFFTMemoryKrolon={space={{},{}},orders={},[true]={},[false]={},safe={}} end
		local N,inverse,mem,twiddles,helper1,helper2,oldLayer,newLayer,V,A,B,Ar,Aim,Br,Bim,r,im,permutation,size=2^math.ceil(math.log(#input+(input[0] and 1 or 0),2)),inverse or false,UniqueNameFFTMemoryKrolon
    ...
    KL_dft=function(input,inverse)
        ...
        local N,vector,inverse,mem,dft,k=#input+(input[0]~=nil and 1 or 0),{},inverse or false,UniqueNameDFTMemoryKrolon
